...
- Potvrzení přidání bota: MessageBox (vrácen na žádost uživatele) + řádek v logu. Tlačítko **Add All** (`buttonAddAll`, vedle Add Bot) přidá bota se stejným nastavením na všechny klienty v seznamu — sdílená logika `ValidatePlaySettings` + `AddBotForClient(title, ...)` (vrací null nebo důvod přeskočení), jeden souhrnný MessageBox.
- **Shutdown PC when done** (`ShutdownWhenDone` checkbox v řadě tlačítek, nepersistuje se záměrně): `RemoveBotFromList(botID, finishedByBot)` — minihry volají s `true`; když poslední bot skončí sám a checkbox je zaškrtnutý → `shutdown /s /t 60` + log + zvuk; odškrtnutí do 60 s → `shutdown /a`. Ruční Stop (menu/Stop All) shutdown nespouští.
